...
## Storage modes (tooling guidance)

The specification remains file-based and append-only, but tooling MAY choose where ledger records are persisted:

- Workspace mode: write `.ai-ledger/contracts/*` and `.ai-ledger/entries/*` in the active working tree.
- Detached git mode: write those same paths on a dedicated ledger branch/worktree (for example `ai-ledger/log` managed under `.git/.ai-ledger/worktree`) so user feature branches are not polluted by ledger-only commits.

In either mode, the append-only rule applies to ledger history itself.

When tooling migrates from workspace mode to a detached git mode, a typical flow is:

1. Discover any existing `.ai-ledger/contracts/*` and `.ai-ledger/entries/*` in the workspace.
2. Copy them into the ledger branch/worktree (committing as needed).
3. Optionally delete the migrated workspace copies (and their now-empty `contracts` / `entries` directories), leaving the ledger branch as the canonical history.

Tooling MAY also:

- Provide a `check`/validation command that inspects the configured storage location (workspace or ledger branch) and fails CI if required contracts/entries are missing or empty.
- Scaffold or update CI workflows to call that command instead of hard-coding assumptions about where `.ai-ledger/` lives.
